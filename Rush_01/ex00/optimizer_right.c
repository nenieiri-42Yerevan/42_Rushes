...
/*   Updated: 2022/10/15 13:29:25 by vismaily         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (input_matrix[3][++j] != 0)
	{
		...
		while (--k != -1)
		{
			...
			if ((size - input_matrix[3][j] + t) >= *z)
				break ;
			cut_values(helper_matrix[j][k], size - input_matrix[3][j] + t, \
					size);
			++t;
		}
	}
...
static int	optimizer_right_line(int **map_matrix, int ***helper_matrix, \
									int j, int size)
{
	int	k;
	...
	while (map_matrix[j][++k] != 0)
	{
		...
		if (helper_matrix[j][k] == 0)
			continue ;
		if (if_in(helper_matrix[j][k], size - k, size) == 0)
			return (-1);
